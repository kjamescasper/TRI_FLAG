"""
run_gen1.py — Generation 1 launcher for TRI_FLAG + ACEGEN

Changes from run_gen0.py:
  - BATCH_ID          = "gen_001"
  - GENERATION_NUMBER = "1"
  - SKIP_SIMILARITY   = "0"   (live ChEMBL/SureChEMBL calls enabled)
  - ENABLE_DEEPPURPOSE = "1"  (S_act fourth reward component active)

Run from triage_agent/ directory:
    python acegen_scripts/run_gen1.py
"""
import os
import sys
import ctypes
...
# ---------------------------------------------------------------------------
os.environ["TRIFLAG_BATCH_ID"]              = "gen_001"
os.environ["TRIFLAG_GENERATION_NUMBER"]     = "1"
...
print(f"[run_gen1] Scorer configured:")
...
if not os.path.exists(_ACEGEN_SCRIPT):
    print(f"\n[run_gen1] ERROR: ACEGEN script not found at:\n  {_ACEGEN_SCRIPT}")
    print("Edit _ACEGEN_SCRIPT path in run_gen1.py to point at your acegen-open clone.")
    sys.exit(1)

print(f"\n[run_gen1] Launching ACEGEN reinvent script:")
